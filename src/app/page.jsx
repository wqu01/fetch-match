...
import {
  Button, Form, Select, SelectItem, Input, Card, CardHeader, CardBody, CardFooter, Image, Pagination, Drawer, DrawerContent, DrawerHeader, DrawerBody, DrawerFooter, useDisclosure, Modal, ModalContent, ModalBody, ModalHeader
} from "@heroui/react";
...
export default function Home() {
  ...
  //favorites
  const { isOpen: isFavOpen, onOpen: onOpenFav, onClose: onCloseFav, onOpenChange: onOpenFavChange } = useDisclosure();
  ...
  return (
    <>
      ...
      <Modal isOpen={isModalOpen} onOpenChange={onOpenModal} onClose={onCloseModal}>
        <ModalContent>
          {(onCloseModal) => (
            <>
              <ModalHeader className="flex flex-col gap-1">{`You matched with ${match.name}`}!</ModalHeader>
              <ModalBody>
                <Card>
                  <CardBody className="overflow-visible p-0">
                    <Image
                      ...
                      width="100%"
                    />
                    <small className="text-default-500">{match.breed}</small>
                    <h4 className="font-bold text-large">{match.name}</h4>
                    <p className="text-tiny font-bold">{match.age} years old</p>
                    <small className="text-default-500">Located in zip code: {match.zip_code}</small>
                  </CardBody>
                </Card>
              </ModalBody>
            </>
